...
- [סיכום הפרויקט](#סיכום-הפרויקט)
- [1. מתודולוגיה](#1-מתודולוגיה)
- [2. קריאה איכותית לפי טיקר](#2-קריאה-איכותית-לפי-טיקר)
- [3. תוצאות הבק-טסט](#3-תוצאות-הבק-טסט)
  - [3.1 הגדרות](#31-הגדרות)
  - [3.2 מספרים מובילים](#32-מספרים-מובילים)
  - [3.3 עקומת Equity](#33-עקומת-equity)
  - [3.4 עקומות למידה](#34-עקומות-למידה-אבחון-מודל)
  - [3.5 מגבלות](#35-מגבלות--להתייחס-ברצינות)
- [4. מה לא עבד](#4-מה-לא-עבד)
- [5. מה הייתי עושה עם יותר זמן / נתונים](#5-מה-הייתי-עושה-עם-יותר-זמן--נתונים)
- [6. שני צינורות — קודם NLP קלאסי, אחר כך LLM](#6-שני-צינורות--קודם-nlp-קלאסי-אחר-כך-llm)
  - [6.1 למה התחלתי דווקא ב-NLP קלאסי](#61-למה-התחלתי-דווקא-ב-nlp-קלאסי--התוכנית-מלמטה-למעלה)
  - [6.2 Pipeline 1 — NLP קלאסי](#62-pipeline-1--nlp-קלאסי-נקודת-ההתחלה-הפשוטה-והזולה)
  - [6.3 ניסויי ביניים](#63-ניסויי-ביניים--הגשר-בין-pipeline-1-ל-pipeline-2)
  - [6.4 Pipeline 2 — LLM מקצה לקצה](#64-pipeline-2--llm-מקצה-לקצה-השדרוג-עם-טכניקות-מתקדמות)
  - [6.5 השוואה זה-לצד-זה](#65-השוואה-זה-לצד-זה-במטריקות-משותפות)
  - [6.6 מסקנה משני הצינורות](#66-מסקנה-משני-הצינורות)
  - [6.7 איפה שני הצינורות נופלים](#67-איפה-שני-הצינורות-נופלים--ניתוח-לפי-שיחה)
- [שחזור הצינור](#שחזור-הצינור)

## ה-gold standard שמשמש בפרויקט הזה

ה-**gold standard** של הפרויקט הוא **התשואה העודפת הריאלית מול SPY** באופק 5
ימים — תשואת המניה בפועל אחרי השיחה פחות תשואת SPY באותו חלון, מעוגן ב-T+1
(בטוח מ-look-ahead). זה מה שכל חיזוי נמדד מולו, וזה מה שכל מספר accuracy /
F1 / AUC / MCC / IC / R² בכתב הזה מחושב ממנו. בכל פעם שהכתב הזה מצטט מספר
כמו "דיוק כיווני 65%", הוא נמדד מול ה-gold standard הזה (תשואה עודפת מול
SPY).
...
**בדיקה ישירה על 6 השיחות הכושלות במשותף.** כדי לוודא שחילוץ הסנטימנט
עצמו לא היה אשם, השוויתי את תיוגי הסנטימנט של הפיפליין מול תיוג עצמאי של
**Claude Opus 4.7** על אותן 6 שיחות — באמצעות אותו prompt `extract_v2`,
אבל ממשפחת מודלים שונה מ-Qwen3-14B של Pipeline 2. שתי קבוצות התיוגים תאמו
זו את זו בצורה הדוקה: **Pearson correlation של 0.89 על `overall_tone`,
וכל 6 השיחות אומתו עצמאית כשוריות.** הפיפליינים לא הוטעו על ידי ציוני
הטון שלהם; הקריאה השורית תואמת את חומר המקור וגם תואמת את מה ש-judge חזק
יותר מפיק עצמאית. זה שולל את האפשרות ש-"חילצנו סנטימנט בצורה שגויה"
כסיבה לכישלונות המשותפים ומשאיר את חוסר פיצ'רי הקונטקסט הסקטוריאלי כהסבר
היחיד. **צוואר הבקבוק לא בחילוץ שלנו.**
